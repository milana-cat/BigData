...
def main():
    filepath = "C:/Users/alexs/Downloads/BigData/BigData/Self work 2/1/flavors_of_cacao.csv"  #Исключить квалити и выделить цветом. 
    ...
    apply_kernel_pca(df.drop(columns=['Rating']), df['Rating'])
    
    # Применение t-SNE
    apply_tsne(df.drop(columns=['Rating']), df['Rating'])
    
    # Анализ линейного PCA
    analyze_linear_pca(df.drop(columns=['Rating']), df['Rating'])
    kpca = KernelPCA(n_components=3, kernel ='cosine');
    X_scaled = kpca.fit_transform(df)
